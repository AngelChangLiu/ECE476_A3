...
void saxpyCuda(int N, float alpha, float* xarray, float* yarray, float* resultarray) {
    ...
    //
    // ECE476 TODO: allocate device memory buffers on the GPU using cudaMalloc.
    //
    ...
    // Allocate memory on the GPU for x, y, and result
    ...
    //
    // ECE476 TODO: copy input arrays to the GPU using cudaMemcpy
    //
    ...
    //
    // ECE476 TODO: copy result from GPU back to CPU using cudaMemcpy
    //
    ...
    //
    // ECE476 TODO: free memory buffers on the GPU using cudaFree
    //

    // Free GPU memory now that we are done with it
    cudaFree(device_x);
    cudaFree(device_y);
