...
test_times = 3 # For grading, we will test with 10 runs and take the best time to reduce noise
perf_points = 1.25
...
def run_tests():
    ...
    for element_count in element_counts:
        ...
        student_runs = [get_time(student_cmd) for _ in range(test_times)]
        your_times[element_count] = min(t for t in student_runs if t is not None)
        print(f"Student Times ({test_times} runs): {student_runs}")
        print(f"Student Min Time: {your_times[element_count]}")

        # Get reference time
        ref_cmd = f"./cudaScan_ref -m {test} -i random -n {element_count} | tee ./logs/ref/{test}_time_{element_count}.log | grep 'Student GPU time:'"
        ref_runs = [get_time(ref_cmd) for _ in range(test_times)]
        fast_times[element_count] = min(t for t in ref_runs if t is not None)
        print(f"Ref Times ({test_times} runs): {ref_runs}")
        print(f"Ref Min Time: {fast_times[element_count]}")
