...
void usage(const char* progname) {
    ...
    printf(
        "  -m  --test <TYPE>      Run specified function on input.  Valid tests are: scan, find_repeats (default: "
        "scan)\n");
...
// #define PARALLEL
    ...
    int* padded = new int[rounded];
    memset(padded, 0, rounded * sizeof(int));
    memmove(padded, start, N * sizeof(int));

    // upsweep phase
    for (int twod = 1; twod <= rounded / 2; twod *= 2) {
        int twod1 = twod * 2;
        for (int i = 0; i < rounded; i += twod1) {
            padded[i + twod1 - 1] = padded[i + twod - 1] + padded[i + twod1 - 1];
        }
    }
    ...
    // downsweep phase
    for (int twod = rounded / 2; twod >= 1; twod /= 2) {
        int twod1 = twod * 2;
        for (int i = 0; i < rounded; i += twod1) {
            int tmp = padded[i + twod - 1];
            padded[i + twod - 1] = padded[i + twod1 - 1];
            padded[i + twod1 - 1] = tmp + padded[i + twod1 - 1];
        }
    }

    memmove(output, padded, N * sizeof(int));
